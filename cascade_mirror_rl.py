...
class CascadeQ(CascadeNN):
    def __init__(self, dim_input, dim_output, init_nb_hidden):
        super().__init__(dim_input, dim_output, init_nb_hidden)
    ...
    def merge_q(self, old_output_model):
        ...
        self.output.weight.data[:, :-1] += old_output_model.weight
...
def main():
    ...
    cascade_qfunc = CascadeQ(dim_s, nb_act, init_nb_hidden=0)
    nb_iter = 200
    ...
    for iter in range(nb_iter):
        ...
        with torch.no_grad():
            ...
            obs_q = cascade_qfunc.get_q(obs).take_along_dim(dim=1, indices=act)
        ...
        cascade_qfunc.add_neurone(obs_feat)
        if iter:
        ...
        grad_steps = 0
        data_loader = DataLoader(TensorDataset(obs_feat, act, rwd, nobs_feat, nact, obs_q, nobs_v, nobs_old_distrib.probs, not_terminal),
        ...
        while grad_steps < min_grad_steps_per_iter:
            ...
            for s, a, r, sp, ap, oldq, oldvp, oldprobp, n_ter in data_loader:
                optim.zero_grad()
                qs = cascade_qfunc.forward_from_old_cascade_features(s).take_along_dim(dim=1, indices=a) + oldq
                vsp = (cascade_qfunc.forward_from_old_cascade_features(sp) * oldprobp).sum(1, keepdim=True) + oldvp
                target = r + gamma * vsp * n_ter
                loss = loss_fct(qs, target)
        ...
        with torch.no_grad():
            ...
            print(f'grad_steps {grad_steps} q_error_train {np.mean(train_losses)} kl {kl} entropy (in (0, 1)) {normalized_entropy}')
