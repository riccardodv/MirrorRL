...
default_config = {
        ...
        "distill_epochs": 100,
        "loss_mixture": [1,0,0,0,0,0,0,0,0]
    }
 



def distill(cascade_qfunc, data, eta, gamma, distilled_cascade = None, distill_epochs = 10, loss_mixture = [1,0,0,1,1,0,0,0,0], force_computation=False, print_log = False):
    print("____DISTILLING____")
    if distilled_cascade is not None:
        new_cascade = distilled_cascade
    else:
        new_cascade = SimpleCascadeQ(cascade_qfunc.dim_input, cascade_qfunc.dim_output, [3*cascade_qfunc.nb_hidden, cascade_qfunc.nb_hidden], with_batchnorm = True)
    ...
    device = cascade_qfunc.output.weight.device
    ...
    for i in range(distill_epochs):
        ...
        if print_log and force_computation:
...
def ce_loss(x, eta, cascade1, cascade2, detach = True, temperature=TEMPERATURE):
    t = temperature
    old_cascade_features = cascade1.get_features(x)
    new_cascade_features = cascade2.get_features(x)
    sm = nn.Softmax(dim = 1)
...
def run(config, checkpoint_dir=None, save_model_dir=None):
    ...
    for iter in range(nb_iter):
        ...
        if iter in when_to_distill:
            new_cascade = distill(cascade_qfunc, data, eta, gamma, distill_epochs=distill_epochs, loss_mixture=loss_mixture, force_computation=True, print_log=True)
            # new_cascade = distill(cascade_qfunc, roll, eta, gamma, distill_epochs=distill_epochs, loss_mixture=loss_mixture, force_computation=True, print_log=True)
            old_obs = old_data["obs"].to(device)
            new_obs = roll["obs"].to(device)
            print("final CE loss on old data: {}".format(ce_loss(old_obs, eta, cascade_qfunc, new_cascade)))
            print("final CE loss on new data: {}".format(ce_loss(new_obs, eta, cascade_qfunc, new_cascade)))

            print("final out loss on old data: {}".format(output_loss(old_obs, cascade_qfunc, new_cascade)))
            print("final out loss on new data: {}".format(output_loss(new_obs, cascade_qfunc, new_cascade)))
