...
def run(config, checkpoint_dir=None, save_model_dir=None):
    ...
    for iter in range(nb_iter):
        ...
        for grad_steps, batch_idx in enumerate(bs):
            ...
            if grad_steps % target_update_freq == 0:
                # train_losses = []
                with torch.no_grad():
                    newqsp = cur_Q(nobs).gather(
                        dim=1, index=nact)  # q-value for the next state and actions taken in the next states
                    newdelta = qfunc(nobs).gather(dim=1, index = nact)
                    qsp = cur_Q(obs).gather(dim=1, index = act)
                    q_target = rwd + gamma * (newqsp + newdelta) * not_terminal - qsp # do we really need to update Q-target for each epoch????????????
                    dataset = TensorDataset(obs, act, q_target)
            s, a, tq = dataset[batch_idx]
            optim.zero_grad()
            delta = qfunc(s)
            delta = delta.gather(dim=1, index=a)
            loss = (delta - tq).pow(2).mean()
        ...
        ensemble_qfunc = ensemble_nn.forward_boosting
        cur_Q = ensemble_nn.forward
        ...
        with tune.checkpoint_dir(iter) as checkpoint_dir:
            ...
            torch.save((ensemble_nn.state_dict(), ensemble_nn.state_dict()), path)
    ...
    if save_model_dir is not None:
        torch.save(ensemble_nn, os.path.join(save_model_dir, f'cascade_qfunc_{ENV_ID}.pt'))
