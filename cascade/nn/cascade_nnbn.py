...
#input is not connected directly to output
class CascadeNNBN2(nn.Module):
    def __init__(self, dim_input, dim_output, init_nb_hidden = 1, non_linearity = nn.ReLU()):
        super().__init__()
        ...
        if init_nb_hidden > 0:
            self.cascade_neurone_list.append(CascadeNeuroneBN(nb_in = dim_input, nb_out=init_nb_hidden, non_linearity= non_linearity))
...
#input is connected directly to output
