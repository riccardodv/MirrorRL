...
from cascade.nn import Simple_Cascade


#TODO check that the output layer doesn't change during the training of distillation
#TODO check why distillation makes it worse, maybe va
...
def test_reg_inc(nb_points):
    ...
    distill_every_n_epoch = add_every_n_epoc * 10
    distill_epochs = 100
    ...
    while nsteps < min_nsteps:
        if epoch % distill_every_n_epoch == 0 and epoch != 0:
            print("____DISTILLING____")
            new_cascade = Simple_Cascade(model.dim_input, model.dim_output, [model.nb_hidden])
        ...
        if epoch % add_every_n_epoc == 0:
            ...
            model.add_n_neurones(features, nb_inputs=model.nb_hidden + dim_data_input, n_neurones=nb_added_neurones)  # add just one neuron
